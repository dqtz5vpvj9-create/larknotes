...
export interface WikiNode {
  ...
  obj_type: string;
  title: string;
  /** Unix seconds (as a string) the underlying object was last edited.
   *  Returned by both `wiki nodes list` and `wiki spaces get_node` — this is
   *  the remote-change signal the sync engine keys on, so it can tell which
   *  docs changed without fetching their content. */
...
export class LarkCli {
  ...
  /**
   * Fetch a doc as **v2 XML** (`--doc-format xml --detail with-ids`).
   *
   * XML over markdown: the v2 markdown path renders images as `![](signed-url)`
   * — an expiring URL with no stable token — whereas XML keeps
   * `<image src="<file_token>">`, which the attachment cache needs to download
   * the asset. `--detail with-ids` also tags every block with its id, kept
   * for future block-level sync. The XML is converted to Obsidian markdown
   * locally by `larkXmlToObsidianMarkdown`. Content lives at
   * `data.document.content`.
   */
  async fetchDoc(docToken: string): Promise<string> {
    const r = await this.run([
      "docs", "+fetch", "--doc", docToken,
      "--api-version", "v2", "--doc-format", "xml", "--detail", "with-ids",
    ]);
  ...
  /**
   * Create a doc via the docx **v2** API. v2 has no `--title` flag — the
   * title is derived from the content's leading H1, so we guarantee one.
   * Returns `data.document` (`{ document_id, revision_id, url }`).
   */
  async createDoc(title: string, markdown: string, folderToken?: string) {
    ...
    const r = await this.run(args, { stdin: content });
    return r?.data ?? null;
  }

  /**
   * Overwrite (or append to) a doc via the docx **v2** API. v2 names the
   * operation `--command` and feeds content through `--content` rather than
   * v1's `--mode` / `--markdown`.
   */
  async updateDoc(
    docToken: string,
